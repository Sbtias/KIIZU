-- KIIZU 010: user-created minigames with safe, data-driven mechanics.
...
alter table public.games add column if not exists game_config jsonb not null default '{"mechanic":"click"}'::jsonb;
...
create policy "published games read" on public.games for select using (unlocked_by_default=true or is_published=true or auth.uid()=creator_id);
drop policy if exists "own games update" on public.games;
create policy "own games update" on public.games for update using(auth.uid()=creator_id) with check(auth.uid()=creator_id);
...
begin
 ...
 if p_mechanic not in ('click','reaction','memory') then raise exception 'INVALID_MECHANIC'; end if;
 if p_min<1 or p_max<p_min or p_max>8 then raise exception 'INVALID_PLAYER_LIMITS'; end if;
 insert into games(slug,name,description,min_players,max_players,unlocked_by_default,is_published,creator_id,game_config)
 values('user-'||replace(gen_random_uuid()::text,'-',''),trim(p_name),left(coalesce(p_description,''),500),p_min,p_max,false,false,caller,jsonb_build_object('mechanic',p_mechanic))
...
begin
 ...
 update games set is_published=true where id=p_game_id and creator_id=auth.uid() returning * into result_row;
