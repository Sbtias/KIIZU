...
begin
  ...
  if not exists(select 1 from public.match_players where match_id=p_match_id and user_id=caller) then
    raise exception 'NOT_A_PLAYER';
  end if;

  update public.match_players
  set score=p_score, placement=0
  ...
  if submitted=total then
    ...
    update public.matches
    set status='playing'
    where id=p_match_id and status='starting'
    returning * into m;
