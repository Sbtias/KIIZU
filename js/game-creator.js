...
canvas.addEventListener("pointerdown",e=>{if(e.button!==0)return;if(space){cameraDrag={x:e.clientX,y:e.clientY,px:pan.x,py:pan.y};canvas.setPointerCapture(e.pointerId);return}const p=screenToWorld(e.clientX,e.clientY);if(tool==="select"){const i=hit(p.x,p.y);selected=i>=0?i:null;if(i>=0){drag={i,dx:p.x-entities[i].x,dy:p.y-entities[i].y};canvas.setPointerCapture(e.pointerId)}renderList();draw()}else if(tool==="erase"){const i=hit(p.x,p.y);if(i>=0){entities.splice(i,1);selected=null;renderList();draw()}}else add(tool,p.x,p.y)});
...
addEventListener("keydown",e=>{if(e.code==="Space"){space=true;e.preventDefault()}if((e.key==="Delete"||e.key==="Backspace")&&selected!==null){entities.splice(selected,1);selected=null;renderList();draw()}});addEventListener("keyup",e=>{if(e.code==="Space")space=false});
